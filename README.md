...
- **Sichtbarkeit garantiert (seit 2.10.2):** Die Rückleitung trägt einen **Anker** – bei Erfolg zum Erfolgsbereich (`#gfb-erfolg-{formId}`), im Fehlerfall zum Formular (`#gfb-form-{formId}`); der Browser scrollt nativ, ohne JavaScript. Zusätzlich zeigt das Frontend bei Erfolg ein viewport-zentriertes **Quittungs-Overlay** (Häkchen, «Erfolgreich übermittelt»), das aktiv geschlossen werden muss (Knopf, Esc oder Klick daneben); beim Schliessen springen Fokus und Scroll zum Erfolgsbereich (`role="status"`). `prefers-reduced-motion` wird respektiert. Mit gewählter Folgeseite (Danke-Seite) gibt es weder Anker noch Overlay – dort ist die Seite selbst die Bestätigung.
