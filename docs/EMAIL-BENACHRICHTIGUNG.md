...
**Bestätigungs-Knopf in der Link-Mail:** Der Block «Bestätigungs-Knopf» (nur im Container «Bestätigungslink-Mail – Inhalt») trägt den Link automatisch – er hat bewusst kein URL-Feld, weil der Gutenberg-Link-Dialog den Platzhalter `{{bestaetigungslink}}` nicht als URL akzeptiert. Beschriftung und Farben sind anpassbar; in Textabsätzen funktioniert der Platzhalter weiterhin. Fehlt beides, hängt der Server den Link automatisch an.

**Form des Bestätigungslinks:** Der Link in der Mail ist eine Frontend-URL der Website (`https://ihre-domain/?gfb-bestaetigung={Nummer}&gfb-token={Token}`) – bewusst keine wp-admin-Adresse (Phishing-Optik, Mailfilter-Abstrafung). Ältere Mails mit der früheren admin-post-URL funktionieren unverändert weiter (Alias auf denselben Kern).
